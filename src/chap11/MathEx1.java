...
/*Math 클래스 : 수치 계산 기능을 하고 있는 메서드들의 모임
 * 			메서드 : static 메서드
 * 			상수 : PI (원주율),
 * 				 E (자연로그)
 * 
...
public class MathEx1 {
	public static void main(String[] args) {
		...
		System.out.println("Math.rint(-5.4)=" + Math.rint(-5.4));
		// min,max : 최소, 최대값 (실수도 가능)
		System.out.println("Math.min(5.4)=" + Math.min(5, 4));
		System.out.println("Math.min(-5.4)=" + Math.min(5.4, 5.3));
		System.out.println("Math.max(-5.4)=" + Math.max(5, 4));
		System.out.println("Math.max(-5.4)=" + Math.max(5.4, 5.3));
		// 반올림
		...
		// 삼각함수 (각도의 단위가 라디안)
		System.out.println("Math.PI/2()=" + Math.sin(Math.PI / 2));
